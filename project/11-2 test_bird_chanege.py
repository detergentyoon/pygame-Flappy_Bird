...
def check_collision(pipes):
    ...
    for pipe in pipes:
        if score <= 2:
            ...
            if yellowbird_rect.colliderect(pipe):
                death_sound.play()
                can_score = True
                return False
        elif 6 <= score:
            if redbird_rect.colliderect(pipe):
                death_sound.play()
                can_score = True
                return False

    if bluebird_rect.top <= -100 or bluebird_rect.bottom >= 900:
        death_sound.play()
        can_score = True
        return False
        
    elif yellowbird_rect.top <= -100 or yellowbird_rect.bottom >= 900:
        death_sound.play()
        can_score = True
        return False

    elif redbird_rect.top <= -100 or redbird_rect.bottom >= 900:
        death_sound.play()
        can_score = True
        return False
...
def bird_animation():
    new_bird = bluebird_frames[bird_index]
...
# 1. 재시작 후 첫번째 파이프 통과 시에만 점수 획득 logic 이 무시됨
#       soulution => 새 변화 logic 수정 시 해결 될 것 같음

# 2. 새 색깔 변화 시 centery 값이 순간적으로 튀어오르며 그 값이 너무 클 경우에는 파이프 이미지를 뚫고 통과하기도 함

# 3. 새 색깔 변화 직전 점수 대에서 파이프 통과 시 이전 단계와 다음 단계 두 가지의 rect 값이 적용되어 2점을 획득하고 파이프에 부딫히지 않았는데도 게임 오버됨
#       soulution => 파이프 점수 획득 구간(centerx) 를 파이프 중간 단계가 아니라 파이프 끝 쪽으로 변경 (만약 변화 도중의 rect 값이 파이프와 부딫혀서 죽는거라면 이걸로 해결 가능)

# 4. 3단계에서 게임 오버 시 드문 확률로, 중력 오버클럭이 생겼을 때처럼 재시작을 해도 바로 게임 오버되는 현상 발생
#       soulution => 우선 1, 2, 3 버그를 해결하고 나면 자연스럽게 해결 될 것 같음



################################################################# 추가 요소 목록 ####################################################################

# 1. 일정 스코어 이후 파이프 색깔 변경

# 2. 게임 시작화면 (message.png)