...
기존 스코어링 방식 = 시간 단위로 득점  
현재 스코어링 방식 = 새의 중심점이 파이프 중심점 사이 좌표를 지나쳤을 때 득점

또한 이 스코어링 방식으로 변경하면서, 상단 파이프와 하단 파이프 두 개가 개별적으로 스코어링되면서 2점씩 올라가는 버그가 있었습니다.

하지만 이 버그는 화면 밖으로 나간 파이프를 지우는 디버그 과정에서 동시에 해결되었습니다. 하지만 나는 이것이 어떤 구조에서 해결된 것인지 이해할 수가 없습니다.

### **화면 밖으로 나간 파이프를 지우는 함수**
```python
def move_pipes(pipes):
    for pipe in pipes:
        pipe.centerx -= 5
